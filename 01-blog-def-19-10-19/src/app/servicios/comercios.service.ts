...
export class ComerciosService {
  ...
  /*findOneComercioByDate(mes: number): Observable<Comercio[]> {
    return this.http.get<Comercio[]>(`${this.url}/${mes}/comercio`).pipe(
      catchError(e => {
        console.error(e.error.mensaje);
        Swal.fire('error al buscar:', e.error.mensaje, 'error')
        return throwError(e);
  ...
  findOtros(): Observable<Comercio[]> {
    return this.http.get<Comercio[]>(`${this.url}/otros`).pipe(
      catchError(e => {
        console.error(e.error.mensaje);
        Swal.fire('error al buscar:', e.error.mensaje, 'error')
        return throwError(e);
  ...
  create(comercio: Comercio): Observable<any> {
    return this.http.post<any>(`${this.url}/crear`, comercio, { headers: this.httpHeaders }).pipe(
      catchError(e => {
        console.error(e.error.mensaje);
  ...
  delete(id: number): Observable<Comercio> {
    return this.http.delete<Comercio>(`${this.urlDelete}/${id}`,{ headers: this.httpHeaders }).pipe(
      catchError(e => {
   ...
   addLike(id: number): Observable<Comercio> {
    ...
    subirFoto(archivo: File, archivo1:File, archivo2:File, id): Observable<Comercio>{
  ...
  return this.http.post(`${this.url}/upload`, formData).pipe(
    map((response : any) => response.comercio as Comercio),
    catchError(e =>{
      console.error(e.error.mensaje);
      Swal.fire( e.error.mensaje, e.error.error, 'error' );
    return throwError(e);
  ...
  subir1Foto(archivo: File, id, id_img): Observable<Comercio> {
    let formData = new FormData();
    formData.append("archivo", archivo);
    formData.append("id", id);
    return this.http.post(`${this.url}/uploadOneFoto/${id_img}`, formData).pipe(
      map((response: any) => response.comercio as Comercio),
      catchError(e => {
        console.error(e.error.mensaje);
        Swal.fire(e.error.mensaje, e.error.error, 'error');
